...
| DEP-2b | Module smoke-load with real dev env                       | Complete    | Phase 3      | follows module + router load (no circular import); validation exercised. NOTE: a full browser click-through vs a live seeded Mongo was not run in this environment — HTTP layer is covered by 21 Supertest integration tests + service unit tests; recommend a manual `pnpm dev` pass before prod deploy |
